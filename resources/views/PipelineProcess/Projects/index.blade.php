...
<div id="loader" class="text-center waitingShield" data-slug="" style="display: none;">
    <div>
...
<div>
    <div class="flex flex-col w-0 flex-1 overflow-hidden" style="display: contents;">
        ...
        <main tabindex="0" class="flex flex-col flex-1 relative z-0 pt-6 focus:outline-none">
            <div class="flex items-center mb-4">
                <h1 class="text-2xl font-semibold text-gray-900">Projects</h1>
            </div>
            <div class="container mx-auto flex flex-col flex-1 px-0 sm:px-0 md:px-0 py-4">
                <div class="flex flex-col sm:flex-row flex-col-reverse items-center justify-between mb-6">
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <div class="relative flex-grow focus-within:z-10">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg
                                        viewBox="0 0 20 20" class="h-5 w-5 text-gray-400 fill-current">
                                        <path
                                            d="M12.9 14.32a8 8 0 1 1 1.41-1.41l5.35 5.33-1.42 1.42-5.33-5.34zM8 14A6 6 0 1 0 8 2a6 6 0 0 0 0 12z">
                                        </path>
                                    </svg>
                                </div> 
                                <input placeholder="Search.." class="filter-name form-input block w-full rounded-none rounded-l-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                            </div>
                            <div class="relative inline-block text-left">
                                <div>
                                    <button class="btn btn-gray leading-6 sm:text-sm sm:leading-5 -ml-px rounded-l-none filter-project">
                                        <svg viewBox="0 0 20 20" class="h-4 w-4 text-gray-400 fill-current">
                                            <path d="M12 12l8-8V0H0v4l8 8v8l4-4v-4z"></path>
                                        </svg> 
                                        <span class="ml-2">Filter</span>
                                    </button>
                                </div>
                                <div class="origin-top-right absolute mt-2 w-auto rounded-md shadow-lg z-40 right-0 show-filter"
                                    style="display: none;">
                                    <div class="dropdown-menu p-4">
                                        <form action="{{ url('pipeline-process/projects') }}" method="get">
                                        <div class="form-group mb-6">
                                            <label class="form-label">Status</label>
                                            <select class="form-select select-status" name="status">
                                                <option value="">All</option>
                                                <option @if($status == 'ongoing') selected @endif value="ongoing">Ongoing</option>
                                                <option @if($status == 'overdue') selected @endif value="overdue">Overdue</option>
                                        ...
                                        <div class="form-group">
                                            <label class="form-label">Visibility</label>
                                             <select class="form-select select-status" name="visibility1">
                                                <option value="">All</option>
                                                <option @if($visibility == '2') selected @endif value="2">Only me</option>
                                                <option @if($visibility == '1') selected @endif value="1">Team Members</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
                    ...
                    <div class="flex items-center justify-end w-full mb-4 sm:mb-0 sm:w-1/2 -mx-2">
                        {{-- <div class="px-0 sm:px-2">
                            <button class="btn btn-blue" id="delete-project">
                                Delete
                            </button>
                        </div> --}}
                        <div class="px-0 sm:px-2">
                            <button class="btn btn-blue" id="openModal">
                                Create Project
                            </button>
                        </div>
                    </div>
                </div>
                <div class="bg-white shadow overflow-hidden rounded-md" id="filterd-data">
                    @if(count($projects) > 0)
                    <ul>
                        ...
                        <li class="border-b border-gray-100 last:border-0"><a
                                ...
                                <div class="px-6 py-4 flex items-center">
                                    <div class="min-w-0 flex-1 md:grid md:grid-cols-8 md:gap-4">
                                        ...
                                        @php
                                            $pendingTask = PipelineProcessTask::whereIn('column_id',$project['columns']->pluck('id')->toArray())->where('completed_at',NULL)->get()->count();
                                            $no_of_days = 0;
                                            ...
                                            if($today < $start_date){
                                                ...
                                                $no_of_days = $today->diff($end_date)->format("%a");
                                            }
                                        ...
                                        <div class="mt-2 md:col-span-2 sm:mt-0">
                                            <div>
                                                <div class="flex items-center text-sm leading-5 text-gray-500"><svg
                                                    ...
                                                    </svg> <span class="font-medium">@if($pendingTask > 0) {{ $pendingTask }} @else 0 @endif</span>&nbsp;Tasks Left
                                                </div>
                                                <div class="mt-2 flex items-center text-sm leading-5 text-gray-500"><svg
                                                    ...
                                                    <span v-if="project.days_left && project.days_left > 0">
                                                        @if ($end_date <= $today)
                                                        <span class="font-medium text-danger">{{ $no_of_days }} &nbsp;days overdue</span>
                                                        @else
                                                   ...
                                                   $projectMembers = ProjectMember::where('project_id',$project['id'])->get();
                                                    if(count($projectMembers) > 0){
                                                        foreach ($projectMembers as $key => $projectMember) {
                                                            $staff = Staff::where('id',$projectMember->member_id)->first();
                                                    ?>
        ...
        <div class="fixed inset-0 max-h-full overflow-y-auto p-6 z-40" id="ProjectModal" style="background-color: rgba(0, 0, 0, 0.4); display: none;">
            <div class="mx-auto max-w-xl">
                <form action="{{ url('pipeline-process/project-store') }}" method="POST">
                    @csrf
                <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                    <div class="p-6">
                        <h3 class="font-medium text-lg">Create Project</h3>
                    </div>
                    <div>
                        <div class="bg-gray-50 p-6 sm:hidden">
                            <select class="form-select" id="addGeneralTab">
                                <option value="tabGeneral">General Info</option>
                                <option id="team-tab" value="tabTeam" disabled>Team Members</option>
                            </select>
                        </div>
                        <div class="sm:block">
                            <div class="border-b border-gray-200 commonpopupmodel">
                                <ul class="nav nav-tabs flex hideTabmobile" id="myTab" role="tablist">
                                    <li class="nav-item active flex-1">
                                        <a class="nav-link active py-4 px-1 text-center border-b-2 border-transparent font-medium text-sm leading-5 text-gray-600" id="generalInfo-tab" data-toggle="tab" href="#generalInfo" role="tab" aria-controls="generalInfo" aria-selected="true"  tabindex="0">General Info</a>
                                    </li>
                                    <li class="nav-item flex-1">
                                        <a class="nav-link py-4 px-1 text-center border-b-2 border-transparent font-medium text-sm leading-5 text-gray-600 cursor-not-allowed" id="teamMembers-tab" data-toggle="" href="#teamMembers" role="tab" aria-controls="teamMembers" aria-selected="false"  tabindex="1">Team Members</a>
                                    </li>
                                </ul>
                                <div class="tab-content" id="myTabContent">
                                    <div class="tab_pane_general tab-pane fade in active" id="generalInfo" role="tabpanel" aria-labelledby="generalInfo-tab">
                                        <section>
                                            <div class="flex flex-col px-6 pt-6 pb-6">
                                                ...
                                                <div>
                                                    <div class="form-group">
                                                        <label for="project-name" class="form-label">Name</label>
                                                         <input id="project-name" name="name" placeholder="Project Name" class="form-input">
                                                ...
                                                <div>
                                                    <div class="form-group">
                                                        ...
                                                        <input id="project-description" name="description" placeholder="Project Description" class="form-input">
                                                ...
                                                <div>
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-input flatpickr-input" name="timeline"
                                                            id="rangeDate" placeholder="YYYY-MM-DD to YYYY-MM-DD" type="text"
                                                ...
                                                <div class="mb-6">
                                                    <div><label class="block text-sm font-medium leading-5 text-gray-700">Project
                                                            Color</label>

                                                        <div class="flex flex-wrap items-center mt-1 -mx-1">
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(252, 129, 129)"
                                                            ...
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(246, 173, 85)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                                    style="background-color: rgb(246, 173, 85);"></a></div>
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(104, 211, 145)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                                    style="background-color: rgb(104, 211, 145);">
                                                                   </a></div>
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(79, 209, 197)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                                    style="background-color: rgb(79, 209, 197);">
                                                                   </a></div>
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(99, 179, 237)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                                    style="background-color: rgb(99, 179, 237);">
                                                                   </a></div>
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(127, 156, 245)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                                    style="background-color: rgb(127, 156, 245);">
                                                                   </a></div>
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(183, 148, 244)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                                    style="background-color: rgb(183, 148, 244);">
                                                                   </a></div>
                                                            <div class="px-1"><a href="javascript:void(0)" rgb-color="rgb(246, 135, 179)"
                                                                    class="selectColor bg-gray-600 flex items-center text-white text-xs justify-center w-6 h-6 overflow-hidden rounded-md focus:outline-none focus:shadow-outline-blue"
                                                ...
                                                <div>
                                                    <div class="relative flex items-start">
                                                        <div class="form-check"><input id="visibility-only-me" type="radio" value="2" name="visibility"
                                                                class="form-radio" ></div>
                                                        <div class="pl-7 text-sm leading-5"><label for="visibility-only-me"
                                                    ...
                                                    <div class="relative flex items-start mt-4">
                                                        <div class="form-check"><input id="visibility-team-members" type="radio" value="1" name="visibility"
                                                                class="form-radio"></div>
                                                        <div class="pl-7 text-sm leading-5"><label for="visibility-team-members"
                                                                class="font-medium text-gray-700">
                                                                Team Members
                                    ...
                                    <div class="tab_pane_team tab-pane fade" id="teamMembers" role="tabpanel" aria-labelledby="teamMembers-tab">
                                        <section>
                                            <div class="flex flex-col" style="height: 530px;">
                                                <div class="px-6 pt-6"><input placeholder="Search Users.." class="form-input staff_members_search"></div>
                                                <div class="mt-4 pb-2 flex flex-col overflow-y-auto" id="filter-staff-member">
                                                    <input type="hidden" name="staff_member_id[]" id="staff_member_array">
                                                    <a href="javascript:void(0)"
                                                        class="flex items-center px-6 py-3 hover:bg-gray-100 select-all">
                                                            <div class="flex items-center justify-center w-5 h-5 p-1 overflow-hidden rounded-full border checkbox_div1">
                                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="w-4 h-4 fill-current show_checkbox1" style="display: none;" >
                                                    ...
                                                    <a href="javascript:void(0)"
                                                        class="flex items-center px-6 py-3 hover:bg-gray-100 staff_member" data-id="{{$staff['id']}}">
                                                            <div
                                                                class="flex items-center justify-center w-5 h-5 p-1 overflow-hidden rounded-full border checkbox_div">
                                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="w-4 h-4 fill-current show_checkbox" style="display: none;" ><path d="M0 11l2-2 5 5L18 3l2 2L7 18z"></path></svg>
                                                            </div>
                                                        <div class="flex flex-col ml-6">
                                                    ...
                                                    @endforeach
                                                    <input type="hidden" id="user-data" value="{{ implode(',',$staff_id) }}">
                                                </div>
                                            </div>
                                        </section>
                                    </div>
                    ...
                    <div class="bg-gray-50 p-4 sm:px-6 sm:py-4 sm:flex sm:flex-row-reverse"><button type="submit"
                            class="btn btn-blue" id="submit-project">
                            Save Project
                        </button> <button type="button" class="btn btn-flat mr-3 hideModal">
                            Cancel
                        </button></div>
                </div>
            </form>
...
@section('required-script-for-this-page')
    ...
    var staff_member_id=[];
      $(document).ready(function(){
        ...
        //     tags: true,
        // })
        $('.filter-project').click( function(e) {  
            e.preventDefault();
            e.stopPropagation();
            $('.show-filter').show();
            $('.show-filter .dropdown-menu').show();
        });    
        ...
        $(document).click(function(){
            $('.show-filter').hide();
            $('.show-filter .dropdown-menu').hide();
        });
      ...
      $('#visibility-only-me').on('click',function(){
        staff_member_id = [];
        ...
        console.log($('input[name="staff_member_id[]"]').val());
    ...
    $(".staff_members_search").on("keyup", function() 
    {
    ...
    $(document).on('click',".project_checkbox",function()
    {
            ...
            if($(this).prop("checked") == true){
                project_id_array.push(project_id);
            }
            else if($(this).prop("checked") == false){
                var index = project_id_array.indexOf(project_id);
                project_id_array.splice(index,1);
            }
    });
    ...
    {
        swal({
        title: "Deleting the project will delete its associated tasks as well. Are you sure?",
        type: 'warning',
        allowEscapeKey: false,
        showCancelButton: true,
        confirmButtonText: 'Yes (Delete project and its tasks)',
        cancelButtonText:'No (Will not do anything)',
        ...
        {  
            ...
            $('#loader').show();
            var site_url = '/pipeline-process/projects/delete-selected-project';
            $.ajax({
            headers: {
            ...
            data:
            {
                project_ids_array: project_id_array,
            },
    ...
    {
        swal("Warning", "Please select aleast one project", "warning");
    }
   });
   ...
   $(".filter-name").keyup(function(){
       var session_data = $('#session-data').val();
        var search = $(this).val();
        var url = '/pipeline-process/project'
        $.ajax({
            headers: {
            'X-CSRF-TOKEN': $('#token').val()
            },
...
$(document).on('change', '#addGeneralTab', function(){
...
$(document).on('click','#submit-project',function(){
    ...
    if((client_id) == ''){
        $("#client_id-error").html('Client name is required');
    }else{
        $("#client_id-error").html('');
    }
    if((project_name) == ''){
    ...
    if(client_id != '' && (project_name) != '' && (project_description) != '' && (rangeDate) != '' && (color) != '' && (team) != undefined){
        $(this).submit();
    }else{
        return false;
    }
