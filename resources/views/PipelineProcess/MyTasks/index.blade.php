        ...
        <main tabindex="0" class="flex flex-col flex-1 relative z-0 overflow-y-auto pt-6 focus:outline-none">
            ...
            <div class="container mx-auto w-full flex flex-col flex-1 px-4 sm:px-6 md:px-8 py-4">
                ...
                <div class="flex flex-col flex-col-reverse md:flex-row mx-0 sm:-mx-6" >
                    ...
                    <div class="flex flex-col flex-1 px-0 sm:px-6" id="mytask">
                        @if ($total_task > 0)
                        
                        @include('PipelineProcess.MyTasks.todays-task')
                        @include('PipelineProcess.MyTasks.upcoming-task')
                        @include('PipelineProcess.MyTasks.overdue-task')
                        @include('PipelineProcess.MyTasks.no-overdue-task')
                        @include('PipelineProcess.MyTasks.completed-task')
                        ...
                        <div class="mb-6">
                            <nav>
                                <a href="{{ url('pipeline-process/tasks') }}"
                                    class="flex items-center px-4 py-2 rounded-lg text-sm mb-1 hover:bg-white bg-white font-semibold"><svg
                                        stroke="currentColor" viewBox="0 0 24 24" class="w-4 h-4 overflow-hidden mr-3">
                                        <path
                                            d="M0 2C0 .9.9 0 2 0h16a2 2 0 0 1 2 2v16a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2zm14 12h4V2H2v12h4c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2z">
                                        </path>
                                    </svg> <span>Inbox</span>
                                </a>
                                <a href="{{ url('pipeline-process/tasks/today') }}"
                                    class="flex items-center px-4 py-2 rounded-lg text-sm mb-1 hover:bg-white">
                                    <div class="w-4 h-4 overflow-hidden mr-3"><svg viewBox="0 0 20 20"
                                ...
                                <a href="{{ url('pipeline-process/tasks/upcoming') }}"
                                ...
                                <a href="{{ url('pipeline-process/tasks/overdue') }}"
                                    class="flex items-center px-4 py-2 rounded-lg text-sm mb-1 hover:bg-white transition duration-150">
                                    <div class="w-4 h-4 overflow-hidden mr-3"><svg viewBox="0 0 20 20"
                                            xmlns="http://www.w3.org/2000/svg" class="w-full h-full fill-current">
                                            <path d="M13 8V0L8.11 5.87 3 12h4v8L17 8h-4z"></path>
                                ...
                                <a href="{{ url('pipeline-process/tasks/no_overdue') }}"
                                    class="flex items-center px-4 py-2 rounded-lg text-sm mb-1 hover:bg-white transition duration-150">
                                    <div class="w-4 h-4 overflow-hidden mr-3"><svg viewBox="0 0 20 20"
                                            xmlns="http://www.w3.org/2000/svg" class="w-full h-full fill-current">
                                            <path d="M13 8V0L8.11 5.87 3 12h4v8L17 8h-4z"></path>
                                ...
                                <a href="{{ url('pipeline-process/tasks/completed') }}"
                                    class="flex items-center px-4 py-2 rounded-lg text-sm mb-1 hover:bg-white transition duration-150">
                                    <div class="w-4 h-4 overflow-hidden mr-3"><svg viewBox="0 0 20 20"
                                            xmlns="http://www.w3.org/2000/svg" class="w-full h-full fill-current">
                                            <path d="M13 8V0L8.11 5.87 3 12h4v8L17 8h-4z"></path>
                        ...
                        <div class="mb-4">
                            ...
                            <nav class="mt-2">
                                @if (count($projects->toArray()) > 0)
                                    ...
                                    <a href="{{ url('pipeline-process/project/tasks') }}/{{ $project->id }}"
...
@section('required-script-for-this-page')
    ...
    var session_data = $('#session-data').val();
        $(document).ready(function() {
            ...
            $(".content-complete").click(function(){
                ...
                var csrf = $("#token").val();
                var id = $(this).siblings('.comment-task-id').val();
                $.ajax({
                    type:'POST',
                    url:site_url,
            ...
            $(document).on('click','.due-date-val',function() {
                var currentclass = $(this);
                // var id = $(this).siblings('.due-date-task-id').val();
                var id = $(this).attr('task-id');
                localStorage.setItem('id', id);
                localStorage.setItem('currentclass', currentclass);

            })
            $(".due-date-val").flatpickr({
                // minDate: "today",
                dateFormat: "Y-m-d",
                disableMobile: "true",
            ...
            $('.assign-Task').click( function(e) {  
                e.preventDefault();
                e.stopPropagation();
                $(this).parent().siblings('.assign-Task-Popup').toggle();
            });    
            $('.assign-Task-Popup').click( function(e) {
                e.stopPropagation();
            });
            $('body').click( function() {
                $('.assign-Task-Popup').hide();
            });
            ...
            $(".comment-on-column").on('click',function(){
                var due_date_id = $(this).attr('data-id');
                var sub_task_id = $(this).attr('sub-task-id');
                var due_date_value = $(this).attr('due-date');
                ...
                //     },
                //     type: "Post",
                //     url: '/pipeline-process/task/popup',
                //     data: {'task_id':due_date_id,'sub_task_id':sub_task_id},
                //     success: function(data)
                //     {
                //         $('.commentOnColumnPopup').html(data)
                //         $('.commentOnColumnPopup').show();
                //     }
                // })
