...
@section('form')
    ...
    @include('ActivityBuilder.form',['data'=>$data])
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function fileSelectProgramVideo(elem) {


            var fileInput = elem.files[0];
            var fileUrl = window.URL.createObjectURL(elem.files[0]);
            $(".tutorial").attr("src", fileUrl);
          ...
          toggleWaitShield('show');
            ...
            $.ajax({
                ...
                success: function(data) {
                   toggleWaitShield('hide');
                   // console.log(data);
                   $("#tutorialVideo").val(data);
                   $("#addProgram").html('<i class="fa fa-times"></i> Remove');
                   $("#uploadProgram").addClass("remove-tutorial-video");
                   // $( "#video-area" ).load(window.location.href + " #video-area" );
