...
@section('form')
    ...
    <!-- end: Delete Form -->
    {!! Form::open(['method' => 'patch', 'route' => ['videos.update', $video->id], 'id'=>'video-form', 'class'=>'margin-bottom-30','data-form-mode' => 'standAlone']) !!}
    @include('ActivityBuilder.videos.form',['video' => $video,'abWorkouts' => $abWorkouts])
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('body').on('click', '.remove-img', function(e){
			...
			$.post('{{url("/")}}'+'/remove/image', {preImg:preImg}, function(response){
				var data = JSON.parse(response);
				if(data.status == 'success')
					$this.remove();
			})
    ...
    <script>
     ...
     $('.saveVideoBtn').click(function(){
         if($('#video-form').valid()){
             ...
             toggleWaitShield("show");
             $.post("{{route('videos.update',['id' => $video->id])}}",formData,function(response){
                 toggleWaitShield("hide");
