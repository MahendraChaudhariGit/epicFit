...
jQuery(function ($) {
    ...
    PT.Ajax = new function () {
        ...
        /* Where to call back */
        //this.APIURL = 'http://192.168.225.50/result/public';
        // this.APIURL = 'http://api.onesportevent.com/DevApi';
        this.APIURL = $('meta[name="public_url"]').attr('content');
                ...
                function (data) {
                    ...
                    if (data.Mode == "Prewritten") {
                      ...
                      //alert();
                           ...
                           //console.log(PT.GeneratePlan(data.Response.Plans));
                         //  alert("not okk");
                    }
    ...
    PT.Switcher = new function () {
        ...
        this.GoToNext = function () {
             ...
             // $("#create_pr").addClass("create_for_design");
             $(".create_for_design").click(function(){
                
                   $("#create_program_hide").show();
                   $("#create_program_design").trigger("click");
    ...
    PT.InitializeWeekDays = function (weekPattern, doRefresh) {
        ...
        for (var i = 1; i <= weekPattern.length; i++) {
            if (weekPattern.charAt(i - 1) == '1') {
                $(".checkbox:nth-child(" + (2 * (i == 1 ? 7 : i - 1) - 1) + ") input", "#weekDays").attr('checked', 'checked');
            }
            else {
                $(".checkbox:nth-child(" + (2 * (i == 1 ? 7 : i - 1) - 1) + ") input", "#weekDays").removeAttr('checked');
            }
        }
    ...
    PT.Start = function (showPreview, webkeyGuid) {
        ...
        PT.State.Gender = $("#fit_gender").val() == "Female" ? 2:1; 
       ...
       //alert(PT.State);
        ...
        $('#planSelector1').on("click", "input[type='image']", function (evt) {
               ...
               var stateName = $(evt.target).parents(".item_class").attr("data-name");
...
var CB = {
    // API_PATH: 'http://192.168.0.50/result/public',
    //API_PATH: 'http://192.168.225.50/result/public',
    // API_PATH: 'http://api.onesportevent.com/DevApi',
    API_PATH : $('meta[name="public_url"]').attr('content'),
    platformWebKey: '',
    IsSetup: false,
    ...
    UI: {
               ...
               //console.log(h3s);                
                // h3s.click(function () {
                  //  jQuery(this).next().slideToggle();
                    ...
                    //return false;
                //}).next().hide();
               // console.log(h3s.find('a'));
                // / h3s.find('a').click(CB.UI.addExercise);
                //console.log(h3s.find('a').click(CB.UI.addExercise));
                  CB.UI.addExercise();
        ...
        addProgramInList: function(p){
            ...
            jQuery(CB.UI.isMobile?'<a href="javascript:void(0)">' :'<tr>')
                //.attr({'data':jQuery.toJSON({id:p.FixedProgramId})})//, name:p.ProgramName, desc:p.ProgramDesc
                ...
                .append('<td>' + p.ProgramName + '</td>\
                    ...
                    <td>' + FX.name + '</td>\
                    <td>\
                        <a class="btn btn-xs btn-default tooltips" href="#" data-placement="top" data-original-title="Edit" data-toggle="modal" data-target="#customPlanUpdateModal">\
                            <i class="fa fa-pencil link-btn"></i>\
                        </a>\
                        <a href="#" class="btn btn-xs btn-default tooltips open-step" data-placement="top" data-original-title="View" data-target-step="trainingSegment">\
                            <i class="fa fa-share link-btn"></i>\
                        </a>\
                    </td>\
                </tr>')
                /*.append('<td>' + p.ProgramName + '</td><td>\
				    <button class="btn btn-default open-step" data-target-step="trainingSegment">Action</button>\
				</td></tr>')//<td class=" w70"> <button class="btn btn-default create_for_design" onclick="CB.UI.goTo(\'manage\',this)"
                /*.click(function(){
            ...
            if(pn.replace(/ /g,'').length > 0){
                CB.FX.createProgram(pn, function(response){
                    if(response.MessageId == 0){
                        ...
                        //PT.Switcher.GoToNext();
                        //CB.UI.startManage();
                        programChoosedMedhod = 1;
                    }
                });
            }
            else 
                CB.showErrorDiv('.pts .error-msg');
        },
        ...
        addExercise: function () {
          ...
          // alert("fe");
          CB.UI.initExSearch();
         //   CB.UI.aexTo = jQuery(this).parents('h5');
           // popup.show('plugins/fitness-planner/custom/add_exercise.html?r=' + Math.random(), jQuery.extend({}, CB.UI.popupParams, { onload: CB.UI.initExSearch }));
           // e.stopPropagation();
        },
        ...
        initExSearch: function () {
           ...
           // alert("ew");
            ...
            //console.log($('.act-btns').find('a'));
            $('.act-btns a').click(function () {
             //alert("ef");
                ...
                alert("fwerewrews");
                    ...
                    CB.FX.addExToProgram({ FixedProgramID: CB.selectedProgram.id, Workout: ca[0].replace('-', ' '), ExerciseTypeID: ex.id }, function (response) {
                        ...
                        if (response.MessageId == 0) {
                            ...
                            sv.find('#' + ex.id + ' .right img')[1].src = 'fitness-planner/images/icon/added.png';
                            CB.UI.backToSearch();
                            jQuery('.cat_det').sortable('refresh');
        ...
        searchExercise: function (contnue) {
           // alert("fe");
            ...
            var elmsg = elc.find('.result-msg');
                ...
                //alert("DFREF");
                //console.log(jQuery('#search_b').find('input').val());
                console.log(jQuery('#search_b').val());
            CB.FX.searchExercise({ keyWords: /*jQuery('#search_b').find('input').val()*/jQuery('#search_b').val(), category: jQuery("#pt-category").val(), equipment: jQuery('#pt-equipment').val(), ability: jQuery('#pt-ability').val(), bodypart: jQuery('#pt-bodypart').val(), myFavourites: jQuery('#pt-favorite')[0].checked, perPage: 10, pageNumber: pagenum }, function (response) {
    ...
    FX: {
        ...
        loadUserSession: function(){
            //DONE
            localStorage['CB.userEmail'] = FX.email;//'t';
            localStorage['CB.sessionGUID'] = FX.SessionGuid;//'590EC926-0942-481E-B11D-3666A9FBF157';
        },
        loadProgramsList: function(){
            //DONE
            var url = CB.API_PATH + 'CustomPlan/GetUsersPlans';//?jsoncallback=?
            CB.APICallback(url, {}, function(response){
                customPlanDatatable.fnDestroy();
                ...
                {
                    for(var i=0; i<plans.length; i++){
                        /*listProgramdata.push({
                        id:plans[i].FixedProgramId,
                        updatedAt:plans[i].DateChanged,
                        name: plans[i].ProgramName
                        //desc:plans[i].ProgramDesc
                        });*/
                ...
                /*$('#client-datatable').DataTable({
                    // data: listProgramdata,id: p.FixedProgramId, name: p.ProgramName, desc: p.ProgramDesc
                    "bDestroy": true
                });*/
                
                customPlanDatatable = customPlanTable.DataTable()
            });
        },
        createProgram: function(name, callback){
            //DONE
            var url = CB.API_PATH + 'CustomPlan/CreateProgram',//?jsoncallback=?
                data = {name:name, description:'', snippet:''};
                
            CB.APICallback(url, data, callback);
        },
        removeWorkoutwithExercise: function (plan_id,id, callback){
            //DONE
            var url = CB.API_PATH + 'CustomPlan/RemoveWorkoutWithExercise';//?jsoncallback=?
            CB.APICallback(url, {fixedProgramExerciseID:plan_id,workout:id}, callback);
        },
        loadProgram: function(id, callback){
			...
			//var copy_data = CB.FX.createProgramCopy(id);
			
			
			var url1 = CB.API_PATH + 'CustomPlan/CreateProgramCopy',//?jsoncallback=?
                data1 = {fixedProgramId:id};

            CB.APICallback(url1, data1, function(response1){
                // console.log(response1.Program.FixedProgramId);return false;
				
               // console.log(copy_data);return false;
			
				var url = CB.API_PATH + 'CustomPlan/GetUsersPlanDetail',//?jsoncallback=?
					data = {fixedProgramId:response1.Program.FixedProgramId};
		...
		createProgramCopy: function(id, callback){
			// alert(id+'abhi');
            var url1 = CB.API_PATH + 'CustomPlan/CreateProgramCopy',//?jsoncallback=?
                data1 = {fixedProgramId:id};
            ...
            CB.APICallback(url1, data1, function(response1){
                ...
                // var url2 = CB.API_PATH + 'CustomPlan/GetUsersPlanDetail',//?jsoncallback=?
                // data2 = {fixedProgramId:response1.Program.FixedProgramId};

				// CB.APICallback(url2, data2, function(response2){
					// /*if(typeof callback != 'undefined')
						// callback(response);
					// else{
						// var exs = response.Exercises;
        ...
        updateProgram: function(id, name, desc, callback){
            ...
            var url = CB.API_PATH + 'CustomPlan/UpdateProgram',//?jsoncallback=?
                data = {name:name, description:desc, snippet:'', fixedProgramId:id};
            CB.APICallback(url, data, callback);
        },
