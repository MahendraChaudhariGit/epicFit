...
jQuery(function ($) {
    ...
    PT.Start = function (showPreview, webkeyGuid) {
        ...
        PT.State.Gender = $("#fit_gender").val() == "" ? 170 : $("#fit_height").val();   // Height in CMS
...
//console.log(p.id);
        ...
        addProgramInList: function (p) {
          //  console.log(p);
           ...
           // alert("dewdwdwdwdswdsw");
            ...
            .append('<td class=" w70">' + p.ProgramName + '</td><td class="">\
					<button class="btn btn-default create_for_design" href="javascript:void(0)" onclick="CB.UI.goTo(\'manage\',this)">Action</button>\
				</td></tr>')
            .click(function () {
                if (this.tagName.toLowerCase() == 'a')
                    CB.UI.goTo('manage', jQuery(this).find('div')[0]);
            })
            .appendTo(jQuery('.pts .my_pt'));
        },
            ...
            if (pn.replace(/ /g,'').length > 0)
                 ...
                 if (response.MessageId ==0) {
                   ...
                   localStorage['CB.SelectedProgram'] = jQuery.toJSON({ id:response.Program.FixedProgramId,'name': pn, 'desc': '' });
                        // location.href = 'manage.html';
                       // alert("e3");
                      // alert(response.Program.FixedProgramId);
                     // alert(localStorage['CB.SelectedProgram']);
                        PT.Switcher.GoToNext();
                        alert("ewf");
                        CB.UI.startManage();
                    }
                });
            else CB.showErrorDiv('.pts .error-msg');
        },
        ...
        searchExercise: function (contnue) {
           // alert("fe");
            ...
            CB.FX.searchExercise({ keyWords: jQuery('#search_b').find('input').val(), category: jQuery("#pt-category").val(), equipment: jQuery('#pt-equipment').val(), ability: jQuery('#pt-ability').val(), bodypart: jQuery('#pt-bodypart').val(), myFavourites: jQuery('#pt-favorite')[0].checked, perPage: 10, pageNumber: pagenum }, function (response) {
        ...
        loadProgramsList: function () {
            ...
            CB.APICallback(url, {}, function (response) {
                ...
                for (var i = 0; i < plans.length; i++) {
                    ...
                    //console.log(plans[i].ProgramName);
            //console.log(listProgramdata);
