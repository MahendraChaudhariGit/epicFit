...
function fileSelectHandler(elem) {
	...
	//remove previous image atributes like height width
	$oImage.removeAttr('style');
    ...
    $.ajax({
		...
		success: function(response){
			...
			$('.filePreviewCls').val(''); //remove exist image for same file issue
		}
     });
...
function CrmfileSelectHandler(elem) {
	// get selected file
	var oFile = elem.files[0];
	...
	picCropModel.find('button.save').unbind("click").click(function(){
		if(ui_w.val() != ''){
            ...
            form_data['photoName'] = photoName.val();
            form_data['widthScale'] = widthScale.val();
            form_data['x1'] = ui_x1.val();
            form_data['w'] = ui_w.val();
            form_data['heightScale'] = heightScale.val();
            form_data['y1'] = ui_y1.val();
            form_data['h'] = ui_h.val();
            form_data['prePhotoName'] = prePhotoName.val();
			...
			$.ajax({
				...
				success: function(response){
					...
					if(entityIdVal != null && entityIdVal != ''){
						...
						formData['photoName'] = response;
						$.ajax({
							url: public_url+'crm/'+formGroup.find('input[name="saveUrl"]').val(),
							data: formData,                         
							method: 'POST'
						});
					}
						...
						formGroup.find('input[name="'+photoHelperVal+'"]').val(response);
					var crmPath = $('#crmPath').val();
					// alert(crmPath);
					// console.log(crmPath);
					previewPics.prop('src', 'http://'+crmPath+'uploads/thumb_'+response);
					if(previewPics.hasClass('hidden'))
						previewPics.removeClass('hidden');
	...
	picCropModel.on('hide.bs.modal', function(){
		if(!ifCroppedImgSaved){
			...
			formData['photoName'] = photoName.val();
			$.post(public_url+'crm/photos/delete', formData);	
		}
		destroyJcrop();
    });
