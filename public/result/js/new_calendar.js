                            ...
                            if (checkDateExistsBetTwo(calEvent.date) == true)
                            {
                                ...
                                $.get(public_url + 'clientevents/classes/' + eventId, function (data) {
                                   ...
                                   $("#dateAt").html(moment(data.startDatetime).format('ddd, D MMM YYYY [at] h:mm A')); 
                                    ...
                                    if(moment(data.startDatetime).subtract(24, 'hours').format('YYYY-MM-DD HH:mm:ss')<moment().format('YYYY-MM-DD HH:mm:ss')){
                                   ...
                                   /* var locaData={};
                                    locaData['date'] = data.date;
                                    locaData['day'] = moment(data.date).format("dddd");
                                    locaData['time'] = data.startTime;
                                    locaData['duration'] = data.duration;
                                    locaData['eventId'] = eventId;*/


                                    /*if(signup_class_stat.eventType == 'signupClass'){
                                        $.get(public_url+'settings/business/classes/'+data.classId+'/free-areas', locaData, function(data){
                                        //console.log(data);
                                        $.each(data,function(index,value){
                                             $("#locupclass").html(value);
                                        });
...
var resServiceData = "<legend>Service </legend>  <div class='form-group delMsgPar'> <label class='strong'> Service </label> <p> " + data.serviceName + "</p></div> <div class='form-group'><label class='strong'> Duration </label><p> " + data.duration + " mins</p></div> <div class='form-group'><label class='strong'> Price </label><p> " + data.price + "</p></div>";
                                    ...
                                    classModal.find("#details_recur_render").addClass("hidden");                                    classModal.find("#details_recur").removeClass("hidden");
            ...
            eventRender: function (event, element, view) {
                var contentElem = element.find(".fc-content"),
                        // eventHtml = '<div id = "service_' + event.id + '"><input type="hidden" name="type" value="' + event.type + '"><input type="hidden" name="eventId" value="' + event.id + '"></div>',
                        eventHtml = '<input type="hidden" name="type" value="' + event.type + '"><input type="hidden" name="eventId" value="' + event.id + '">',
                        iconHtml = '';

                        // contentElem.parent().addClass("service_" + event.id);
                        contentElem.addClass("service_" + event.id);
            ...
            eventAfterAllRender: function (view) {
                ...
                if (view.type == 'month') {
                    ...
                    $('.fc-content-skeleton tr > td').unbind('click.addEventPopover').bind('click.addEventPopover', function (event) {
                        ...
                        //checkdateObj['date']=$(this).find('input[name="type"]').val();
                        //console.log($(this).find('input[name="type"]').val())
    ...
    function getStaffEvents(staffId, ifInsertNeverEndingEventsRepeat) {
        ...
        } else
        {
            ...
            $.get(public_url + 'client-events', formData, function (data) {
                ...
                /*var todayDate = momentt.format('YYYY')+'-'+momentt.format('MM')+'-'+momentt.format('DD');
                 header.each(function(index, elem){
                 ...
                 coltextHelperFunc($elem, momentt.format('LL'));
...
//if modal not showing disable cell click
    ...
    function show_booking_modal_by_url(eventId)
    {
    // abhi11
    ...
    $.get(public_url + 'clientevents/single-service/' + eventId, function (data) {
        ...
        calEventDate = data.date;
        // calEventDate = '2017-09-23';

        var eventType = 'single-service';
        var $this = $(this),
        eventId = data.id;
        ...
        classModal.find("#appointStatusPending").addClass("hidden");
        classModal.find(".set-group-disp a").removeClass("hidden");                    
        classModal.find("#ifAutoExpireAppoint").addClass("hidden");
        ...
        classModal.find("#nextTab,.submit").removeClass("hidden");
        classModal.find("#unlinkClientClass").addClass("hidden");
        ...
        $this.find('input').remove();
        // alert('#service_'+eventId);
        // alert($('#service_'+eventId).html());

        $('.service_'+eventId).prepend(eventHtml);
        ...
        // alert(data.appointStatusOpt);
                    // alert($("#appointStatusOpt").val());
        if(data.appointStatusOpt == 'Pencilled-In'){
            $("#appointStatusOpt").val();
            classModal.find('input[name="appointStatusOpt"]').val(status);
...
//                                return false;
                                ...
                                if (moment(data.clientAddedAt).add(24, 'hours').format('YYYY-MM-DD HH:mm:ss') > moment().format('YYYY-MM-DD HH:mm:ss')) {
