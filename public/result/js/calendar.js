        ...
        calendar.fullCalendar({
                        ...
                        if(eventType == 'class'){
                            ...
                            if(checkDateExistsBetTwo(calEvent.date) == true){
                                ...
                                $.get(public_url + 'clientevents/classes/' + eventId, function (data) {
                                    ...
                                    if(moment(data.startDatetime).subtract(edit_time_limit, 'hours').format('YYYY-MM-DD HH:mm:ss')<moment().format('YYYY-MM-DD HH:mm:ss')){
                                        ...
                                        //temporary Change
                                        classModal.find('a#unlinkClientClass').data('is-before-24',true);
                                        classModal.find('a#unlinkClientClass').removeClass('hidden');
                                        classModal.find('#show_error').addClass('hidden').html();
                                    }
                                    else{   
                                        classModal.find('a#unlinkClientClass').data('is-before-24',false);
                                        classModal.find('a#unlinkClientClass').removeClass('hidden');
                                        classModal.find('#show_error').addClass('hidden').html();
                                    }
                                    ...
                                    for (var i in data.clients){
                                        ...
                                        if(data.clients[i].id == loggedInUser.id){
                                            var isLdc = data.clients[i].isLdc;
                                            classModal.find('a#unlinkClientClass').data('is-ldc',isLdc);
                                            if(isLdc){
                                                classModal.find('a#unlinkClientClass').removeClass('hidden');
                                            }
                                        }
                            ...
                            if (checkDateExistsBetTwo(calEvent.date) == true)
                            {
                                ...
                                $.get(public_url + 'clientevents/single-service/' + eventId, function (data) {
                                    ...
                                    classModal.find(".delete-prompt-service").data('client-makeup',data.isClientMakeup);
                                    classModal.find(".delete-prompt-service").data('invoice',data.isInvoice);
                                    classModal.find(".delete-prompt-service").data('membership',data.membershipId);
                                    classModal.find(".delete-prompt-service").data('isLdc',data.isLdc);
                                    ...
                                    if(moment(data.startDatetime).subtract(edit_time_limit, 'hours').format('YYYY-MM-DD HH:mm:ss')<moment().format('YYYY-MM-DD HH:mm:ss')){
                                        ...
                                        classModal.find('#service_submit').addClass('hidden');
                                        classModal.find('#show_error').addClass('hidden').html();
                                        classModal.find('a.delete-prompt-service').data('is-before-24',true);
                                        
                                    }
                                    else
                                    {
                                        classModal.find('a.delete-prompt-service').data('is-before-24',false);
                                        classModal.find('a.delete-prompt-service').removeClass('hidden');
            ...
            eventRender: function (event, element, view) {
                ...
                if (event.type == 'appointment') {
                    ...
                    if(event.isLdc)
                        iconHtml += '<span class="epic-tooltip m-l-3" rel="tooltip" data-toggle="tooltip" data-placement="top" title="LDC Class"><img src="'+public_url+'result/images/ldc-icon.png" /></span>';
                ...
                if (view.type == 'agendaDay') {
                    ...
                    var backgroundimg = event.logo;
                    eventHtml +=
                            '<div class="boxwithbg" style="background-image:url('+backgroundimg+')"><div class="crossicon"></div><span class="sessionname">'+eventTitle + '</span><br><span class="eventTimeRange">' +
                            moment(event.start).format("h:mm A") + ' - ' + moment(event.end).format("h:mm A") +
                            '</span><br><span class="tranername"> Trainer - '+event.staffName;'</div>'
                }
...
//resc class 
        ...
        $('body').on('click', '.classAddLnk', function (e) {
            ...
            showClassModal('add');
            $('#classModal').find('#generalTab').hide();
            var selectedDatetimeMoment = moment($('input[name="selectedDatetime"]').val());
            var formattedDate = selectedDatetimeMoment.format('YYYY-MM-DD');
            $('#timepickerDesk').data("DateTimePicker").date(new Date(formattedDate+' 09:00:00'));
        });
                ...
                if(data.length) {
                    $.each(data, function () {
                       ...
                       /* if (this.type == 'busy-time') {
                            ...
                            var pushedIdx = events.push({type: 'single-service', id: this.id, title: this.title, start: eventStart, end: eventEnd, price: this.price, date: this.date,logo:this.logo, isStaffDeleted: this.isStaffDeleted, serviceName: this.serviceName, appointStatusOpt: this.appointStatusOpt, notes: this.appointNote, color: this.serviceColor, staffName: this.staffName, isRepeating: this.isRepeating, clients: []}) - 1;

                                for (var i in this.clients)
                                events[pushedIdx].clients.push(this.clients[i])
                        } else if (this.type == 'class') {
                            var pushedIdx = events.push({type: 'class', id: this.id, title: this.title, start: eventStart, end: eventEnd, price: this.price, date: this.date,logo:this.logo, isStaffDeleted: this.isStaffDeleted, color: this.color, capacity: this.capacity, isRepeating: this.isRepeating, notes: this.notes,isLdc:this.isLdc, staffName: this.staffName, clientsCount: this.clientsCount, clients: []}) - 1;
    ...
    function showClassModal(mode) {
        ...
        if (!rescheduleClientsCount() && !resheduleAppointDateSelMode) {
            ...
            if (changeEventDateDateSelMode) {
                ...
                if (mode == 'add') {
                    classModal.find(".selectStaffDD").show();
                    classModal.find(".setEventTime").show();
                    classModal.find('.availableStaff').show();
                    ...
                    var calendEvent = eventObj.find('.calendEvent');
                    classModal.find(".set-group-disp a").addClass("hidden");
                    classModal.find(".selectStaffDD").hide();
                    classModal.find(".setEventTime").hide();
                    classModal.find('.availableStaff').hide();
                ...
                if(eventType == "single-service"){
                    calendEvent = eventObj.find('.calendEvent');
                    calendEvent.find('input[name="staffName"]').val();
                    classModal.find('.set-group-disp-staff').text(calendEvent.find('input[name="staffName"]').val());
                }
    ...
    function enterReschecduleClientMode() {
        if (!isRescheduleClientMode()) {
            ...
            if(isLdc){
                ...
                var max_date = moment(eventDate).startOf('isoWeek').add('days', num_days).format('YYYY-MM-DD');
                var current_date = moment(eventDate).startOf('isoWeek').format('YYYY-MM-DD');
            }else{
                var max_date = moment(initial_date).add('days', num_days).format('YYYY-MM-DD');
                var current_date = moment().format('YYYY') + '-' + moment().format('MM') + '-' + moment().format('DD');
            }
            ...
            if (data.length) {
                ...
                $.each(data, function () {
                    ...
                    if (this.type == 'classSignup') {
                        var pushedIdx = events.push({type: 'class', id: this.id, title: this.title,logo:this.logo, start: eventStart, end: eventEnd, price: this.price, date: this.date, isStaffDeleted: this.isStaffDeleted, color: this.color, capacity: this.capacity, isRepeating: this.isRepeating, notes: this.notes, staffName: this.staffName, clientsCount: this.clientsCount, clients: []}) - 1;
