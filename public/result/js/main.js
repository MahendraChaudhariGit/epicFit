...
var Main = function() {
	...
	var saa = function() {
		var activeItem = $('#app .main-navigation-menu li.active');
		var sidebar = $("#sidebar");
		if(sidebar.length)
			var	itemOffset = $("#sidebar").position().top
		else
			var itemOffset = 0;
		if(activeItem.length)
			var	itemTop = $(activeItem).position().top + itemOffset + 11;
		else
			var itemTop = 11;
		var sclosed = $('.app-sidebar-fixed.app-sidebar-closed').length;
		$('#active-arrow').remove();
		$('#sidebar').prepend('<span id="active-arrow"></span>');
		...
		if( widowWidth > 755 && widowWidth < 991  ) {

			$('#active-arrow').css({ left: '308px' });
		}
	}
	...
	$('#sidebar .perfect-scrollbar').scroll(function() {
		saa();
	})
	...
	var sidebarHandler = function() {
		...
		elem.on(eventObject, 'a', function(e) {
			...
			if ((!isSidebarClosed() || isSmallDevice()) && (cw < 767 || cw > 991)) {
				return;
			}
			_this = $(this);
	...
	var settingsHandler = function() {
		...
		$('input[name="setting-theme"]').change(function() {
			...
			$('#skin_color').attr("href", "plugins/themes/" + selectedTheme + ".css");
		...
		function defaultSetting() {
			...
			$('#skin_color').attr("href", "plugins/themes/theme-1.css");
			$(".navbar-brand img").attr("src", "images/logo.png");
	...
	var panelToolsHandler = function() {
		...
		$('body').on('click', '.panel-collapse', function(e) {
			...
			if($this.hasClass('closed')){
				...
				var panel_group = $this.data('panel-group');
				if(panel_group){
					$("[data-panel-group='"+panel_group+"']").not($this).each(function(){
						var iconElem = jQuery(this).children("i.fa");
						if(jQuery(iconElem).hasClass('fa-chevron-down')){
							$(this).closest(".panel").children(".panel-body").slideUp(200, function() {
								jQuery(iconElem).addClass('fa-chevron-up').removeClass('fa-chevron-down')
							});
						}
					});
				}
	...
	var togglePanel = function(elem){
		var iconElem = elem.children("i.fa");
		elem.closest(".panel").children(".panel-body").slideToggle(200, function() {
			if(jQuery(iconElem).hasClass('fa-chevron-down'))
				jQuery(iconElem).removeClass('fa-chevron-down').addClass('fa-chevron-up')
			else
				jQuery(iconElem).addClass('fa-chevron-down').removeClass('fa-chevron-up')
		});
	}
