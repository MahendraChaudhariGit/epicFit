...
jQuery(function ($) {
    ...
    PT.Start = function (showPreview, webkeyGuid) {
        ...
        PT.State.Gender = $("#fit_gender").val() == "Female" ? 1:2; 
...
var CB = {
    ...
    UI: {
        ...
        addProgramInList: function(p){
            if(FX.name == 'undefined' || typeof FX.name == 'undefined')
                var name_ad = 'Admin';
            else
                name_ad = FX.name;
        ...
        createProgram: function(f){
            var formData={},
                ...
                page = f.page.value;
                if(page == 'Admin'){
                    localStorage['CB.userId'] = 0;
                    formData.image = $('input[name="programImage"]').val();
                    formData.curr_ability = $('select[name="current_ability"]').val();
                }
                else{
                    formData.image = '';
            ...
            if(pn.replace(/ /g,'').length > 0){
                ...
                CB.APICallback(url, formData, function(response){
                    if(response.MessageId == 0){
                        ...
                        //PT.Switcher.GoToNext();
                        //CB.UI.startManage();
                        console.log(localStorage['CB.SelectedProgram']);
    ...
    FX: {
        ...
        loadUserSession: function(){
            //DONE
            localStorage['CB.userId'] = FX.clientid;
        ...
        addExToProgram: function(exercise, callback){
            ...
            if(typeof exercise.WorkOut == 'undefined')
                exercise.WorkOut = parseInt($('select#exe_workout option:selected').val(),10);
    ...
    APICallback: function(url, data, responseFunc){
        //DONE
        var extraData = {
            Clientid : localStorage['CB.userId'],
