...
function fit_build_fit_planner( $webKeyGUID, $email, $webKeyPassword, $showpreview )
{
    ...
    if( 1 == 1 )
    {
	    ...
	    $url='http://192.168.225.50/result/public/Login/PartnerAuthenticate?email=' . urlencode($email) . '&webKeyGuid=' . urlencode($webKeyGUID) . '&webKeyPassword=' . urlencode($webKeyPassword);
	...
	<script type='text/javascript'>
        jQuery(document).ready( function() {
		    PT.Start( showPreview = [[show_preview]] );
        });
	</script>
EOD;
...
if(!function_exists('fit_enqueue_planner_js'))
{
	function fit_enqueue_planner_js() {
        ...
        fit_enqueue_script( 'fit_planner', plugins_url('/js/PTPlanner.js', __FILE__), false);
