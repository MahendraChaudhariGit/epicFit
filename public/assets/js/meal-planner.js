...
$(document).ready(function(){
	...
	$('.submit_meal').click(function(e){
		...
		if($('input[name=prep_time_hrs]').val() == '' && $('input[name=prep_time_mins]').val() == '') {			
			...
			$(".prep-time-error").parent('.form-group').addClass("has-error");
			$("html, body").animate({ scrollTop: 100 }, "slow");
			 return false;
		 }
		 else{
		 	 $(".prep-time-error").parent('.form-group').removeClass("has-error");
		 }
		 if ($('input[name=cook_time_hrs]').val() == '' && $('input[name=cook_time_mins]').val() == '') {	
			 $('.cook-time-error').css('display','block');
			 $(".cook-time-error").parent('.form-group').addClass("has-error");
			 $("html, body").animate({ scrollTop: 100 }, "slow");
			return false;
		}
		else{
			$(".cook-time-error").parent('.form-group').removeClass("has-error");
		}
	 /* end */
		if(isValidName){
			var form = $('#meal-form'),
        ...
        console.log('ingredient_data_1===',ingredient_data_1);
			...
			$('.list-2').each(function(){
				let index = $(this).attr('data-id');
                let item_name = $(this).find('.food-match-2-'+index).val();
				if(item_name){
					ingredient_data_2[item_name]={
					'qty': $(this).find('.food-qty-2-'+index).val(),
					'measure': $(this).find('.food-measure-2-'+index).val(),
					'item': item_name,
			      }
				}	
			});
          ...
          // console.log('ingredient_table==',ingredient_table);
			...
			if(isFormValid){
				...
				if(isImage){
					...
					$('.ingredient-help-block').parent('.form-group').removeClass('has-error');
					$('.ingredient-help-block-2').css('display','none');
					$('.ingredient-help-block-2').parent('.form-group').removeClass('has-error');
					$('.preparation-help-block').css('display','none');
					$('.preparation-help-block').parent('.form-group').removeClass('has-error');
					$('.preparation-help-block-2').css('display','none');
					$('.preparation-help-block-2').parent('.form-group').removeClass('has-error');	 
					$('.ingredient-name-1-help-block').css('display','none');
					$('.ingredient-name-1-help-block').parent('.form-group').removeClass('has-error');
					$('.ingredient-name-2-help-block').css('display','none');
					$('.ingredient-name-2-help-block').parent('.form-group').removeClass('has-error');
					...
					switch(format_type) {
						case '1': 								
								if(ingr_1 == 0){
									...
									$('.ingredient-help-block').parent('.form-group').addClass('has-error');
								}
								if(prep_1 == 0){
									$('.preparation-help-block').css('display','block');
									$('.preparation-help-block').parent('.form-group').addClass('has-error');
								}
						...
						case '2': 				    
								...
								if(ingr_2 == 0){
									...
									$('.ingredient-help-block-2').parent('.form-group').addClass('has-error');
								  }
								if(prep_1 == 0){
								   $('.preparation-help-block').css('display','block');
								   $('.preparation-help-block').parent('.form-group').addClass('has-error');
								...
								if(ingr_name_1==0){
									...
									$('.ingredient-name-1-help-block2').parent('.form-group').addClass('has-error')
								 }
								 if(ingr_name_2==0){
									$('.ingredient-name-2-help-block').css('display','block');
									$('.ingredient-name-2-help-block2').parent('.form-group').addClass('has-error')
								 }
						...
						case '3':					    	
								...
								if(prep_2 == 0){
									...
									$('.preparation-help-block-2').parent('.form-group').addClass('has-error');
								  }	 
							   if(ingr_name_1 == 0){  
									$('.ingredient-name-1-help-block').css('display','block');
									$('.ingredient-name-1-help-block2').parent('.form-group').addClass('has-error')
							   ...
							   if(ingr_1 == 0 || ingr_2 == 0 || prep_1 == 0 || prep_2 == 0 || ingr_name_1== 0 || ingr_name_2 == 0 ){
							    	$("html, body").animate({ scrollTop: 100 }, "slow");
									return false;
								}
						 	break;
					...
					formData['ingr_textarea_1'] = $('#ingredients').val();
					formData['ingr_textarea_2'] = $('#ingredients-2').val();
		...
		success : function(response) {
	        ...
	        if(response.status == 'ok'){
				...
				if(mealData.total_hrs){
					$('#preprationTimeHrs').text(mealData.total_hrs);
					$('.time-hrs').css('display','inline-block');
				}
				if(mealData.total_mins){
					$('#preprationTime').text(mealData.total_mins);
					$('.time-min').css('display','inline-block');
				}
				$('.bottom_data').html(mealData.html);
...
$('body').on('change','.mealName',function(){
	...
	$('.analyze_data').click(function(){
		...
		// var yield = parseInt($('.serving').val());
	...
	//   $.post(public_url+'meal-planner/ingredients', {title:title,ingr:newInge,yield:yield}, function(data){
		$.post(public_url+'meal-planner/ingredients', {title:title,ingr:newInge,form:form}, function(data){
		
		  $('.show-all-list-div').html(data.html);
	...
	// $('.delete-ingr').click(function(){
	$(document).on('click','.delete-ingr',function(){
		var id =$(this).attr('data-id');
		// var id =$('.delete-ingr').attr('data-id');
		var ingr = $("#ingredients").val().split(/\r\n|\n|\r/);
		var remove_item = $('.full-name-'+id).val();
	    ingr.splice( $.inArray(remove_item, ingr) ,1 );
		var new_ingr = ingr.join("\r\n");
		$("#ingredients").val(new_ingr);
		// $('.list-0').remove();
		$('.analyze_data').trigger("click");

	});

	$(document).on('click','.updateRecipeIngr',function(){
		// var id =$('.updateRecipeIngr').attr('data-id');
		var id =$(this).attr('data-id');
		var ingr = $("#ingredients").val().split(/\r\n|\n|\r/);
		var remove_item = $('.full-name-'+id).attr('data-old');
		var add_item = $('.full-name-'+id).val();
	...
	$('.analyze_data-2').click(function(){
		// $('.ingredient-help-block').css('display','none');
		var title= $("#name").val(),
	    //    ingr = $("#ingredients").val().split(/\n|\r/);
		ingr = $("#ingredients-2").val().split(/\r\n|\n|\r/);
		var newInge =[];
		var newInge =[];
		$.each(ingr,function(key,value){
			newValue= value.trim();
		...
		var form = "ingredient-form-2";
		// var yield = parseInt($('.serving').val());
		// var yield = $('#serves').val();
	  $.post(public_url+'meal-planner/ingredients', {title:title,ingr:newInge,form:form}, function(data){ 
		  $('.show-all-list-div-2').html(data.html);
		  ingr_form_1 = $("#ingredients").val().split(/\r\n|\n|\r/);
		  var final_array = $.merge( ingr_form_1, ingr );
		  var finalInge =[];
		$.each(final_array,function(key,value){
			newValue= value.trim();
			data = removeTags(newValue);
			if(data != "" && data != false){
				finalInge.push(data);
	  ...
	  $.post(public_url+'meal-planner/ingredients', {title:title,ingr:finalInge}, function(data){
	...
	// $('.delete-ingr').click(function(){
		$(document).on('click','.delete-ingr-2',function(){
			var id =$(this).attr('data-id');
			// var id =$('.delete-ingr').attr('data-id');
			var ingr = $("#ingredients-2").val().split(/\r\n|\n|\r/);
			var remove_item = $('.full-name-2-'+id).val();
			ingr.splice( $.inArray(remove_item, ingr) ,1 );
			var new_ingr = ingr.join("\r\n");
			$("#ingredients-2").val(new_ingr);
			// $('.list-0').remove();
			$('.analyze_data-2').trigger("click");
	
		});
	
		$(document).on('click','.updateRecipeIngr-2',function(){
			// var id =$('.updateRecipeIngr').attr('data-id');
			var id =$(this).attr('data-id');
			console.log('id', id);
			var ingr = $("#ingredients-2").val().split(/\r\n|\n|\r/);
			var remove_item = $('.full-name-2-'+id).attr('data-old');
			var add_item = $('.full-name-2-'+id).val();
			console.log('ingr',ingr, 'remove_item',remove_item, 'add_item', add_item);
			ingr.splice(ingr.indexOf(remove_item), 1, add_item)
			var new_ingr = ingr.join("\r\n");
			$("#ingredients-2").val(new_ingr);
			$('.analyze_data-2').trigger("click");
		...
		$(document).on('click','.updateRecipe-2',function(){
			var id =$(this).attr('data-id');
			var ingr = $("#ingredients-2").val().split(/\r\n|\n|\r/);
			var remove_item = $('.full-name-2-'+id).attr('data-old');
			/*  */
			let ingr_val = '';
			ingr_val +=	$(".food-qty-2-"+id).val();
			ingr_val += ' ';
			ingr_val +=	$(".food-measure-2-"+id).val();		
			ingr_val += ' ';
			ingr_val +=	$(".food-match-2-"+id).val();
