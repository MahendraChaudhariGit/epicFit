          ...
          var string = '<div class="row">\
                    <div class="panel panel-white exercise" data-duration="'+value.EstimatedTime+'">\
                      <div class="panel-body">\
                        <div class="row">\
                          <div class="col-md-1">\
                            <img src="'+public_url+'assets/plugins/fitness-planner/images/lunge.png" class="mw-70p">\
                          </div>\
                          ...
                          <div id="segment_editbtn_'+value.EditWorkoutId+'" class="col-md-2 p-t-10" data-exercise-id="'+value.ExerciseID+'" data-editworkout="'+value.EditWorkoutId+'">\
                  ...
                  <div class="row treningSegCls hidden" id="segment_'+value.EditWorkoutId+'">\
                    ...
                    <div class="col-md-9 form-inline">\
                      ...
                      <div class="form-group">\
                        ...
                        <input type="text" value="'+value.TempoDesc+'" class="form-control custom-form-control numb-colon" id="exercTempo" name="exercTempo" required="required">\
                      </div>\
                      <div class="form-group">\
...
/* start: Calculate and display plan total time */
  function programTotalTime(){
    ...
    if($('#choosedTrainingsAccordion:visible').length > 0){
      $('#choosedTrainingsAccordion').find('form.treningSeg-form').each(function(){
        var panal = $(this).closest('.panel:visible');
        if(panal.length > 0 && !$(this).closest('.panel').hasClass('marked-to-delete')){
          var $this = $(this),
              sets = parseInt($this.find('input[name="exercSets"]').val()),
              rest = parseInt($this.find('input[name="exercRest"]').val()),
              dur = parseInt($this.find('input[name="exercDur"]').val()),
              reps = parseInt($this.find('input[name="exercReps"]').val()),
              temp = $this.find('input[name="exercTempo"]').val();
          ...
          if(dur > 0){
            totSecs += (sets * dur) + (rest * (sets - 1)); 
          }
          else if(reps > 0 && temp != ''){
            var tempArr = temp.split(':'),
                totalTemp = 0;
            $.each(tempArr, function(i, value){
              totalTemp += parseInt(value);
            })

            totSecs += ((reps * totalTemp) * sets) + (rest * (sets - 1));      
          }
        }
      });
    }
    ...
    if(totSecs)
      var totalMin = Math.round(totSecs/60);
    else
      var totalMin = 0;
