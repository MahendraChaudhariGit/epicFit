	...
	FBPhotoBox.prototype = {
		init: function() {
			...
			$(".fbphotobox-close-btn a").click(function() {
                $(".fbphotobox-main-container").css({"z-index": 9999999999});
                $(".app-sidebar-fixed #sidebarNew").css({"z-index": 9999999999});
				$this.hide();
				return false;
			});
			...
			$('.' + this.settings.containerClassName).on("click", "." + this.settings.imageClassName, function() {
				first_post_id = $(this)[0].dataset.id;
				$this.show($(this));
			});
		...
		show: function(image) {
			var next_image_post_id = image[0].dataset.id;
			if(first_post_id != next_image_post_id){
				if(first_post_id > next_image_post_id){
			...
			this.fbpMainImage.attr('data-id', image.attr('data-id'));
		...
		hide: function() {
			...
			this.fbpMainContainer.fadeOut({easing:'easeOutQuint', queue:false});
			this.fbpOverlay.hide({easing:'easeOutQuint', queue:false});
		...
		initDOM: function() {
			var html = ['<div class="fbphotobox-main-container">',
							'<div class="fbphotobox-container-left">',
								...
								'<div class="fbphotobox-close-btn" style="position:absolute;z-index:9999">',
									'<a title="Close" href="" style="float:left;padding:8px">',
										'<img src="../assets/images/close.png" style="height: 18px;width: 18px;max-width: none !important;"/>',
									'</a>',
