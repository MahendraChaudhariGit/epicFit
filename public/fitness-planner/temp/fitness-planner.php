...
function fit_build_fit_planner( $webKeyGUID, $email, $webKeyPassword, $showpreview )
{
    ...
    if( 1 == 1 )
    {
	    ...
	    $url='http://api.onesportevent.com/DevApi/Login/PartnerAuthenticate?email=' . urlencode($email) . '&webKeyGuid=' . urlencode($webKeyGUID) . '&webKeyPassword=' . urlencode($webKeyPassword);
