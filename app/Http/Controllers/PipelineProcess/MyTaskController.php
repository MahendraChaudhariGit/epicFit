...
class MyTaskController extends Controller
{
    ...
    {
        $staffs = Staff::with('user')->OfBusiness()->get();
//        dd($staffs,auth()->user()->id);
        ...
        // dd($sub_task);
        $todays = PipelineProcessTask::query();

        $todays = $todays->where(function ($query) {
        ...
        $upcomings = PipelineProcessTask::where(function ($query) {
            $query->where('assign_by', auth()->user()->id)
                ->orWhere('original_user_id', auth()->user()->id);
        })
        ...
        $no_due_dates = PipelineProcessTask::where(function ($query) {
            $query->where('assign_by', auth()->user()->id)
                ->orWhere('original_user_id', auth()->user()->id);
        })
            ->whereIn('task_id', $sub_task)->with(['parent', 'parent.column', 'parent.comments', 'parent.clients' => function ($q) {
        ...
        $overdues = PipelineProcessTask::where(function ($query) {
            $query->where('assign_by', auth()->user()->id)
                ->orWhere('original_user_id', auth()->user()->id);
        })
            ->whereIn('task_id', $sub_task)->with(['parent', 'parent.column', 'parent.comments', 'parent.clients' => function ($q) {
...
//            $qry = str_replace(array('%', '?'), array('%%', '%s'), $overdues->toSql());
//            $qry = vsprintf($qry, $overdues->getBindings());
//            dd($qry);
    ...
    public function filterTask($time)
    {
        ...
        $todays = null;
        if ($time === 'today') {
            $todays = PipelineProcessTask::where(function ($query) {
                $query->where('assign_by', auth()->user()->id)
                    ->orWhere('original_user_id', auth()->user()->id);
            })
                ->whereIn('task_id', $sub_task)->with(['parent', 'parent.column', 'parent.comments', 'parent.clients' => function ($q) {
        ...
        $completes = null;
        if ($time === 'completed') {
            $completes = PipelineProcessTask::where(function($query) {
                        $query->where('assign_by', auth()->user()->id)
                            ->orWhere('original_user_id', auth()->user()->id);
                })
                ->whereIn('task_id', $sub_task)->with(['parent', 'parent.column', 'parent.comments', 'parent.clients' => function ($q) {
