...
class ProjectController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $visibility = $request->visibility1;
         ...
         $staff_id = Staff::OfBusiness()->pluck('id')->toArray();
    ...
    public function projectDetail(Request $request, $slug){
        ...
        $filter_assign_user = $request->filter_client;
        ...
        $staffs = Staff::with('user')->OfBusiness()->get()->toArray();
        $staff_id = Staff::OfBusiness()->pluck('id')->toArray();
        $all_clients = Clients::select(DB::raw("CONCAT(clients.firstname,' ',clients.lastname) as name"),'id')->OfBusiness()->get()->toArray();
        $clients = Clients::select(DB::raw("CONCAT(clients.firstname,' ',clients.lastname) as name"),'id')->OfBusiness()->whereNotIn('id',$client_id)->get()->toArray();
        if(count($request->filter_client) > 0 && $request->filter_client[0] != 'all'){
            
            
            $project = Project::with(['columns'=>function($q){
                $q->orderBy('index','asc');
            },'columns.pipeline_process_tasks'=>function($q) use($filter_assign_user){
                $q->whereIn('user_id',$filter_assign_user);
            },'columns.pipeline_process_tasks.comments','projectMember'])
                ->where('slug', $slug)
                ->firstOrFail();
        }else{
            $project = Project::with(['columns'=>function($q){
                $q->orderBy('index','asc');
            },'columns.pipeline_process_tasks','columns.pipeline_process_tasks.comments','projectMember'])
                ->where('slug', $slug)
                ->firstOrFail();
        }
        if($project){
            return view('PipelineProcess.Projects.project-detail',compact('all_clients','project','clients','staffs','staff_id','slug','filter_assign_user'));
        }else{
